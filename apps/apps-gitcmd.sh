UTLAPPS_gitcmd_init(){
  ...
  alias git-branchfrom="UTLAPPS_gitcmd_branchfrom"
  alias git-rmignore="UTLAPPS_gitcmd_rmignore"
}
...
UTLAPPS_gitcmd_branchfrom(){
  ...
  (
    ...
    if [ -z "${lBranch}" ] ; then
      echo you must pass existing branch name; return
    fi

    if [ -z "${newBranch}" ] ; then
      echo you must pass new branch name; return
    fi

    # Check if branch exists
    if ! [ -z "$(git branch --list | grep ${newBranch})" ]; then
        echo "A branch named \"${newBranch}\" already exists dude!"; return
    fi

    # Check if branch is we have pending changes
    if ! [ -z "$(git status --porcelain)" ]; then
      echo "you still have pending changes dude!"; return
    fi
